...
def iterate(mode, loader, Refine_model, Base_model, logger, epoch, val_mode='Refine'):
    block_average_meter = AverageMeter()
    average_meter = AverageMeter()
    meters = [block_average_meter, average_meter]
    # switch to appropriate mode
    assert mode == "val", "unsupported mode: {}".format(mode)
    assert val_mode in ['Base', 'Refine'], "unsupported val_mode: {}".format(val_mode)
    ...
    for i, batch_data in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            result = Result()
            if val_mode == 'Base':
                result.evaluate(pred_base.data, gt.data)
                ...
                logger.conditional_save_img_comparison(mode, i, batch_data, pred_base, epoch)
                logger.conditional_save_pred(mode, i, pred_base)
            else:
...
def main():
    ...
    logger = helper.logger(args, step='eval')
    ...
    with torch.no_grad():
        result, is_best = iterate("val", val_loader, Refine_model, Base_model, logger, 0, val_mode=args.mode)
