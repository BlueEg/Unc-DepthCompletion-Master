...
class logger:
    def __init__(self, args, step, prepare=True):
        self.args = args
        output_directory = get_folder_name(args, step)
    ...
    def conditional_save_pred(self, mode, i, pred):
...
def adjust_learning_rate_Tea(lr_init, optimizer, epoch):
    """Sets the learning rate to the initial LR decayed by 10 every 5 epochs"""
    lr = lr_init * (0.5 ** (epoch))
...
def get_folder_name(args, step):
    current_time = time.strftime('%Y-%m-%d@%H-%M')

    assert step in ["step1", "step2", "eval", "test_completion"], "unsupported step: {}".format(step)

    if step == 'step1':
        return os.path.join(args.result, 'MDCnet.criterion={}.lr={}.bs={}.wd={}.jitter={}.time={}'.format(args.criterion,
                            args.lr, args.batch_size, args.weight_decay, args.jitter, current_time))
    elif step == 'step2':
        return os.path.join(args.result, 'AIRnet.criterion={}.lr={}.bs={}.wd={}.jitter={}.time={}'.format(args.criterion,
                            args.lr, args.batch_size, args.weight_decay, args.jitter, current_time))
    elif step == 'eval':
        return os.path.join(args.result, 'eval')
    else:
        return os.path.join(args.result, 'test_completion')
