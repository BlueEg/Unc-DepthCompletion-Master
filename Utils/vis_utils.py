...
def merge_into_row(ele, pred):
    ...
    if 'rgb' in ele:
        ...
        #rgb = cv2.resize(rgb, (1216, 352), interpolation=cv2.INTER_CUBIC)
        rgb = np.transpose(rgb, (1, 2, 0))
        img_list.append(rgb)
    elif 'g' in ele:
        g = np.squeeze(ele['g'][0, ...].data.cpu().numpy())
        #g = cv2.resize(g, (1216, 352), interpolation=cv2.INTER_CUBIC)
...
def repeat_merge_into_row(ele, pred):
    def preprocess_depth(x):
        y = np.squeeze(x.data.cpu().numpy())
        return depth_colorize(y)
    ...
    for i in range(3):
        img = pred[...,i, :, :]
        print(img.shape)
        img_list.append(preprocess_depth(img))
